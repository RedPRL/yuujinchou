...
sig
  ...
  sig
    ...
    (** Th type of caller ids. Each effect may come a caller id for the effect handler to identify the source of the effect. *)
    type caller
  end
  ...
  sig
    ...
    (** The effect [BindingNotFound {caller; prefix}] means that the engine expected at least one binding within the subtree at [path], but could not find any. Modifiers such as {!val:Language.any}, {!val:Language.only}, {!val:Language.none}, and a few other modifiers expect at least one matching binding. For example, the modifier {!val:Language.except}[ ["x"; "y"]] expects that there was already something under the subtree at [x.y]. If there were actually no names with the prefix [x.y], then the modifier will trigger this effect with [prefix] being [Emp #< "x" #< "y"]. See {!type:Modifier.caller} for the argument [caller]. *)
    type _ Effect.t += BindingNotFound : {caller : caller option; prefix : Trie.bwd_path} -> unit Effect.t

    (** The effect [Shadowing {caller; path; former; latter}] indicates that two items, [former] and [latter], are both assigned to the same [path]. Modifiers such as {!val:Language.renaming} and {!val:Language.union} could lead to bindings having the same name, and when that happens, this effect is performed to resolve the conflicting bindings. The effect is continued with the resolution of [former] and [latter]. For example, to implement silent shadowing, one can continue it with the item [latter]. One can also employ a more sophisticated strategy to implement type-directed disambiguation. See {!type:Modifier.caller} for the argument [caller]. *)
    type _ Effect.t += Shadowing : {caller : caller option; path : Trie.bwd_path; former : data; latter : data} -> data Effect.t

    (** The effect [Hook {caller; prefix; hook; input}] is triggered by modifiers created by {!val:Language.hook}. When the engine encounters the modifier generated by {!val:Language.hook}[ hook] while handling the subtree [input] at [prefix], it will perform the effect [Hook {caller; path; hook; input}], which may be continued with the resulting trie. See {!type:Modifier.caller} for the argument [caller]. *)
    type _ Effect.t += Hook : {caller : caller option; prefix : Trie.bwd_path; hook : hook; input : data Trie.t} -> data Trie.t Effect.t
    ...
    (** [exec ~prefix modifier trie] runs the [modifier] on the [trie] and return the transformed trie. It can perform effects [BindingNotFound], [Shadowing], and [Hook].
        ...
        @param caller The caller identity attached to the performed modifier effects. If unspecified, effects come with {!constructor:None} as their caller. Note that this does not affect the resulting trie but only the effects performed by the engine. A typical use case is for the same effect handler to know in what context the effect is performed.
...
sig
  ...
  sig
    ...
    (** The effect [BindingNotFound {caller; prefix}] means that the engine expected at least one binding within the subtree at [path], but could not find any. The selector {!val:Language.only} expects at least one matching binding. For example, the selector {!val:Language.except}[ ["x"; "y"]] expects that there was already something under the subtree at [x.y]. If there were actually no names with the prefix [x.y], then the selector will trigger this effect with [prefix] being [Emp #< "x" #< "y"]. See {!type:caller} for the argument [caller]. *)
    type _ Effect.t += BindingNotFound : {caller : caller option; prefix : Trie.bwd_path} -> unit Effect.t

    (** The effect [Hook {caller; prefix; hook; input}] is triggered by selectors created by {!val:Language.hook}. When the engine encounters the selector generated by {!val:Language.hook}[ hook] while handling the subtree [input] at [prefix], it will perform the effect [Hook {caller; path; hook; input}], which may be continued with the resulting set of data. See {!type:caller} for the argument [caller]. *)
    type _ Effect.t += Hook : {caller : caller option; prefix : Trie.bwd_path; hook : hook; input : data Trie.t} -> DataSet.t Effect.t
    ...
    (** [exec ~prefix modifier trie] runs the [modifier] on the [trie] and return the transformed trie. It can perform effects [BindingNotFound] and [Hook].
        ...
        @param caller The caller of the trie. If unspecified, effects come with {!constructor:None} as their callers. Note that this does not affect the resulting trie but only the effects performed by the engine. A typical use case is for the same effect handler to know which trie it is handling.
        @param prefix The prefix prepended to any path or prefix in the effects, but in reverse. The default is the empty unit path ([Emp]).
  ...
  sig
    ...
    *)
    ...
    (** [modify_visible m] modifies the visible namespace by
        ...
        @param caller the caller id attached to the modifier effects *)
    ...
    (** [include_singleton (p, x)] adds a new binding to both the visible
        ...
        @param caller_visible the caller id attached to the modifier effects when manipulating the visible namespace
        @param caller_export the caller id attached to the modifier effects when manipulating the export namespace *)
    ...
    (** [section p f] starts a new scope and runs the thunk [f] within the scope.
        ...
        @param caller_visible the caller id attached to the modifier effects when manipulating
        the visible namespace to include the content of the section
        @param caller_export the caller id attached to the modifier effects when manipulating
        the export namespace to include the content of the section *)
