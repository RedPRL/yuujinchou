...
type 'a error = ReplacementNotUsed of 'a pattern | EmptyMeet of 'a pattern
[@@deriving show]
...
module Compile : sig
  ...
  let join2_result ~join r1 r2 =
    ...
    | `Matched ns1, `Matched ns2 ->
        let merger _ e1 e2 = Some (join e1 e2) in
        `Matched (M.union merger ns1 ns2)
  ...
  let meet2_result ~meet r1 r2 =
    ...
    | `Matched ns1, `Matched ns2 ->
        ...
        in
        `Matched (M.merge merger ns1 ns2)
  ...
  let rec compile ~join ~meet :
      'a pattern -> ('a compiled_pattern, 'a error) result = function
    | PatWildcard ->
        Ok
    ...
    | PatScope (prefix, prefix_replacement, pattern) -> (
        compile ~join ~meet pattern
        |> Result.map @@ fun m ->
           let prefix_replacement =
             Option.value prefix_replacement ~default:prefix
           in
           fun default path ->
             ...
             | Some remaining -> (
                 ...
                 | `Matched ns ->
                     `Matched
                       ( M.to_seq ns
                       |> Seq.map (fun (r, attr) ->
                              (prefix_replacement @ r, attr))
                       |> M.of_seq ) ) )
    | PatSeq pats ->
        ResultMonad.map (compile ~join ~meet) pats
        |> Result.map @@ fun matchers ->
    ...
    | PatJoin pats ->
        ResultMonad.map (compile ~join ~meet) pats
        |> Result.map @@ fun ms default path ->
           join_result ~join
             (List.to_seq ms |> Seq.map @@ fun m -> m default path)

  and compile_inv ~join ~meet :
      'a pattern -> ('a compiled_pattern, 'a error) result = function
    | PatWildcard ->
        Ok
          (fun default -> function
            | [] -> singleton [] default
            | _ :: _ -> `NoMatch)
    | PatScope (_, Some _, _) as pattern -> Error (ReplacementNotUsed pattern)
    | PatScope (prefix, None, pattern) -> (
        compile_inv ~join ~meet pattern
        |> Result.map @@ fun m default path ->
           match trim_prefix prefix path with
           ...
           | Some remaining -> (
               ...
               | `Matched ns ->
                   `Matched
                     ( M.to_seq ns
                     |> Seq.map (fun (r, attr) -> (prefix @ r, attr))
                     |> M.of_seq ) ) )
    | PatSeq pats ->
        ResultMonad.map (compile_inv ~join ~meet) pats
        |> Result.map @@ fun matchers ->
           let f r m default path =
             match r default path with
             | `NoMatch -> `NoMatch
             | `Matched ns ->
                 join_result ~join
                   ( M.to_seq ns
                   |> Seq.map @@ fun (path, default) -> m default path )
           in
           List.fold_left ~f
             ~init:(fun default path -> singleton path default)
             matchers
    | PatInv pat -> compile ~join ~meet pat
    | PatAttr (default, pat) ->
        compile_inv ~join ~meet pat |> Result.map @@ fun m _ -> m default
    | PatJoin [] as pattern -> Error (EmptyMeet pattern)
    | PatJoin pats ->
        ResultMonad.map (compile_inv ~join ~meet) pats
        |> Result.map @@ fun ms default path ->
           meet_result ~meet
...
let run_ m path : unit matching_result = run m ~default:() path
