...
sig
  ...
  type t = {visible : (data, tag) Trie.t; export : (data, tag) Trie.t}

  (** {1 Name Resolution} *)

  val resolve : Trie.path -> t -> (data * tag) option
  ...
  val include_singleton : ?context_visible:context -> ?context_export:context -> Trie.path * (data * tag) -> t -> t
  ...
  val include_subtree : ?context_modifier:context -> ?context_visible:context -> ?context_export:context -> ?modifier:hook Language.t -> Trie.path * (data, tag) Trie.t -> t -> t
