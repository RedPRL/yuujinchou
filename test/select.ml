...
module DataSet = struct
  include Set.Make (struct type t = data let compare = compare end)
  let pp fmt s =
    s |> iter @@ fun m ->
    Format.fprintf fmt "%a@ " pp_data m
end
...
let test env pattern expected =
  ...
  and expected' = set_of_list expected
  in
  let set' = collect_matched env' pattern in
  if differ_set set' expected' then begin
    Format.printf "@.";
