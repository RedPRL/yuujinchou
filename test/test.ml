...
let () =
  test true (scope [] @@ renaming [ "a" ] [ "b" ]) [ "a" ]
  @@ matched [ ([ "b" ], true) ]
...
let () =
  test true (scope [ "a" ] wildcard) [ "a"; "b" ]
  @@ matched [ ([ "a"; "b" ], true) ]

let () =
  test true (scope [ "a" ] @@ renaming [ "b" ] [ "c" ]) [ "a"; "b" ]
  @@ matched [ ([ "a"; "c" ], true) ]
...
let () =
  test true (scope [ "a"; "b" ] any) [ "a"; "b" ]
  @@ matched [ ([ "a"; "b" ], true) ]

let () =
  test true (scope [ "a"; "b" ] root) [ "a"; "b" ]
  @@ matched [ ([ "a"; "b" ], true) ]
...
let () =
  test true (scope [ "a"; "b" ] @@ renaming [] [ "c" ]) [ "a"; "b" ]
  @@ matched [ ([ "a"; "b"; "c" ], true) ]
...
let () =
  test true (except [ "a"; "b" ]) [ "a"; "c" ]
  @@ matched [ ([ "a"; "c" ], true) ]

let () =
  test true (except [ "a"; "b" ]) [ "c"; "b" ]
  @@ matched [ ([ "c"; "b" ], true) ]
...
let () =
  test true (except_prefix [ "a"; "b" ]) [ "a"; "c" ]
  @@ matched [ ([ "a"; "c" ], true) ]

let () =
  test true (except_prefix [ "a"; "b" ]) [ "c"; "b" ]
  @@ matched [ ([ "c"; "b" ], true) ]
...
let () =
  test true (renaming [ "a"; "b" ] [ "a" ]) [ "a"; "b" ]
  @@ matched [ ([ "a" ], true) ]

let () =
  test true (renaming [ "a"; "b" ] [ "b" ]) [ "a"; "b" ]
  @@ matched [ ([ "b" ], true) ]

let () =
  test true (renaming [ "a"; "b" ] [ "c" ]) [ "a"; "b" ]
  @@ matched [ ([ "c" ], true) ]
...
let () =
  test true (renaming_prefix [] [ "a" ]) [ "a" ]
  @@ matched [ ([ "a"; "a" ], true) ]

let () =
  test true (renaming_prefix [] [ "a" ]) [ "b" ]
  @@ matched [ ([ "a"; "b" ], true) ]
...
let () =
  test true (renaming_prefix [ "a" ] [ "a" ]) [ "a" ]
  @@ matched [ ([ "a" ], true) ]

let () =
  test true (renaming_prefix [ "a" ] [ "b" ]) [ "a" ]
  @@ matched [ ([ "b" ], true) ]
...
let () =
  test true (renaming_prefix [ "a" ] []) [ "a"; "b" ]
  @@ matched [ ([ "b" ], true) ]

let () =
  test true (renaming_prefix [ "a" ] [ "a" ]) [ "a"; "b" ]
  @@ matched [ ([ "a"; "b" ], true) ]

let () =
  test true (renaming_prefix [ "a" ] [ "b" ]) [ "a"; "b" ]
  @@ matched [ ([ "b"; "b" ], true) ]

let () =
  test true (renaming_prefix [ "a" ] [ "b" ]) [ "a"; "c" ]
  @@ matched [ ([ "b"; "c" ], true) ]
...
let () =
  test true (renaming_prefix [ "a"; "b" ] [ "a" ]) [ "a"; "b" ]
  @@ matched [ ([ "a" ], true) ]

let () =
  test true (renaming_prefix [ "a"; "b" ] [ "b" ]) [ "a"; "b" ]
  @@ matched [ ([ "b" ], true) ]

let () =
  test true (renaming_prefix [ "a"; "b" ] [ "c" ]) [ "a"; "b" ]
  @@ matched [ ([ "c" ], true) ]
...
let () =
  test true (renaming_scope [] [ "x" ] @@ renaming [] [ "a" ]) []
  @@ matched [ ([ "x"; "a" ], true) ]

let () =
  test true (renaming_scope [] [ "x" ] any) [ "a" ]
  @@ matched [ ([ "x"; "a" ], true) ]
...
let () =
  test true (renaming_scope [] [ "x" ] wildcard) [ "a" ]
  @@ matched [ ([ "x"; "a" ], true) ]

let () =
  test true (renaming_scope [] [ "x" ] @@ renaming [ "a" ] [ "b" ]) [ "a" ]
  @@ matched [ ([ "x"; "b" ], true) ]
...
let () =
  test true (renaming_scope [ "a" ] [ "x" ] any) [ "a" ]
  @@ matched [ ([ "x" ], true) ]

let () =
  test true (renaming_scope [ "a" ] [ "x" ] root) [ "a" ]
  @@ matched [ ([ "x" ], true) ]
...
let () =
  test true
    (renaming_scope [ "a" ] [ "x" ] @@ renaming [ "a" ] [ "b" ])
    [ "a" ] nomatch
...
let () =
  test true
    (renaming_scope [ "a" ] [ "x" ] @@ renaming [ "b" ] [ "c" ])
    [ "b" ] nomatch
...
let () =
  test true
    (renaming_scope [ "a"; "b" ] [ "x" ] @@ renaming [ "b" ] [ "c" ])
    [ "c" ] nomatch

let () =
  test true (renaming_scope [ "a" ] [ "x" ] any) [ "a"; "b" ]
  @@ matched [ ([ "x"; "b" ], true) ]
...
let () =
  test true (renaming_scope [ "a" ] [ "x" ] wildcard) [ "a"; "b" ]
  @@ matched [ ([ "x"; "b" ], true) ]
...
let () =
  test true (renaming_scope [ "a"; "b" ] [ "x" ] any) [ "a"; "b" ]
  @@ matched [ ([ "x" ], true) ]

let () =
  test true (renaming_scope [ "a"; "b" ] [ "x" ] root) [ "a"; "b" ]
  @@ matched [ ([ "x" ], true) ]
...
let () =
  test true
    (renaming_scope [ "a"; "b" ] [ "x" ] @@ renaming [] [ "c" ])
    [ "a"; "b" ]
  @@ matched [ ([ "x"; "c" ], true) ]
...
let () =
  test true (attr false (attr true (attr false any))) []
  @@ matched [ ([], false) ]

let () =
  test true (attr true (attr false (attr true (attr false any)))) []
  @@ matched [ ([], false) ]

let () =
  test true
    (attr true (attr false (attr true (attr false (attr false any)))))
    []
  @@ matched [ ([], false) ]
...
let () =
  test false (seq [ join [ only [ "a" ]; renaming [ "a" ] [ "b" ] ] ]) [ "a" ]
...
let () =
  test true (seq [ renaming [ "a" ] [ "b" ]; renaming [ "b" ] [ "c" ] ]) [ "a" ]
  @@ matched [ ([ "c" ], true) ]

let () =
  test true (seq [ renaming [ "a" ] [ "b" ]; renaming [ "b" ] [ "c" ] ]) [ "b" ]
  @@ matched [ ([ "c" ], true) ]

let () =
  test true
    (seq [ renaming [ "a" ] [ "b" ]; renaming [ "b" ] [ "c" ] ])
    [ "c" ] nomatch
...
let () =
  test false
    (seq_filter [ join [ only [ "a" ]; renaming [ "a" ] [ "b" ] ] ])
    [ "a" ]
  @@ matched [ ([ "a" ], false); ([ "b" ], false) ]
...
let () =
  test true
    (seq_filter [ renaming [ "a" ] [ "b" ]; renaming [ "b" ] [ "c" ] ])
    [ "a" ]
  @@ matched [ ([ "c" ], true) ]
...
let () =
  test true (join [ any; renaming [ "a" ] [ "b" ] ]) [ "a" ]
  @@ matched [ ([ "a" ], true); ([ "b" ], true) ]

let () =
  test true (join [ any; renaming [ "a" ] [ "b" ] ]) [ "b" ]
  @@ matched [ ([ "b" ], true) ]

let () =
  test true (join [ any; attr false any ]) [ "a" ]
  @@ matched [ ([ "a" ], true) ]

let () =
  test false (join [ any; attr true any ]) [ "a" ]
  @@ matched [ ([ "a" ], true) ]
...
let () =
  test true (meet [ any; attr false any ]) [ "a" ]
  @@ matched [ ([ "a" ], false) ]

let () =
  test false (meet [ any; attr true any ]) [ "a" ]
  @@ matched [ ([ "a" ], false) ]

(* some "real" test cases *)

let () =
  test true (join [ any; renaming_prefix [] [ "M" ] ]) [ "x" ]
  @@ matched [ ([ "M"; "x" ], true); ([ "x" ], true) ]
...
let () =
  test true (join [ only [ "x" ]; only [ "y" ] ]) [ "y" ]
  @@ matched [ ([ "y" ], true) ]
...
let () =
  test true
    (renaming_scope [] [ "M" ] @@ meet [ except [ "x" ]; except [ "y" ] ])
    [ "x" ] nomatch
