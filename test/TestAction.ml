...
let test_none_3 () =
  Alcotest.check_raises "error"
    ...
    (wrap @@ fun () -> Action.run ~union:myunion none Trie.empty)
...
let test_any_3 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound [])
    (wrap @@ fun () -> Action.run ~union:myunion any Trie.empty)
...
let test_only_3 () =
  Alcotest.check_raises "error"
    ...
    (wrap @@ fun () -> Action.run ~union:myunion (only ["x"]) Trie.empty)
...
let test_except_3 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound ["x"])
    (wrap @@ fun () -> Action.run ~union:myunion (except ["x"]) Trie.empty)
...
let test_in_3 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound ["x"])
    (wrap @@ fun () -> Action.run ~union:myunion (in_ ["x"] any) Trie.empty)
...
let test_renaming_3 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound ["x"])
    (wrap @@ fun () -> Action.run ~union:myunion (renaming ["x"] ["z"]) Trie.empty)
...
let test_seq_2 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound [])
    (wrap @@ fun () -> Action.run ~union:myunion (seq [none; any]) (of_list [["x"; "y"], N 10; ["x"; "x"], N 20; ["y"], N 30]))

let test_seq_3 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound [])
    (wrap @@ fun () -> Action.run ~union:myunion (seq [none; none]) Trie.empty)

let test_seq_4 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound ["x"])
    (wrap @@ fun () -> Action.run ~union:myunion (seq [renaming ["x"] ["z"]; only ["x"]]) (of_list [["x"], N 10; ["y"], N 20]))
