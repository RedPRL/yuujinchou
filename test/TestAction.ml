...
let wrap f =
  ...
  try_with f ()
    { effc = fun (type a) (eff : a Effect.t) ->
          match eff with
          | A.BindingNotFound {prefix; _} -> Option.some @@
            fun (k : (a, _) continuation) ->
            discontinue k @@ WrappedBindingNotFound prefix
          | A.Shadowing {path; former; latter; _} -> Option.some @@
            fun (k : (a, _) continuation) ->
...
let test_none_1 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [])
    (wrap @@ fun () -> A.exec none (of_list [["x"], N 10]))

let test_none_2 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [])
    (wrap @@ fun () -> A.exec none (of_list [[], N 10]))

let test_none_3 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound Emp)
    (wrap_error @@ fun () -> A.exec none Trie.empty)

let test_any_1 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [["x"], N 10])
    (wrap @@ fun () -> A.exec any (of_list [["x"], N 10]))

let test_any_2 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [[], N 10])
    (wrap @@ fun () -> A.exec any (of_list [[], N 10]))

let test_any_3 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound Emp)
    (wrap_error @@ fun () -> A.exec any Trie.empty)

let test_any_4 () =
  let t = of_list [[], N 10] in
  Alcotest.(check @@ trie data) "ok"
    t
    (wrap @@ fun () -> A.exec any t)

let test_only_1 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [["x"], N 10])
    (wrap @@ fun () -> A.exec (only ["x"]) (of_list [["x"], N 10; ["y"], N 20]))

let test_only_2 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [["x"; "y"], N 10; ["x"; "x"], N 20])
    (wrap @@ fun () -> A.exec (only ["x"]) (of_list [["x"; "y"], N 10; ["x"; "x"], N 20; ["y"], N 30]))

let test_only_3 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound (Emp #< "x" #< "y"))
    (wrap_error @@ fun () -> A.exec (only ["x"; "y"]) Trie.empty)

let test_only_4 () =
  let t = of_list [["x"], N 10] in
  Alcotest.(check @@ trie data) "ok"
    t
    (wrap @@ fun () -> A.exec (only ["x"]) t)

let test_except_1 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [["y"], N 20])
    (wrap @@ fun () -> A.exec (except ["x"]) (of_list [["x"], N 10; ["y"], N 20]))

let test_except_2 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [["y"], N 30])
    (wrap @@ fun () -> A.exec (except ["x"]) (of_list [["x"; "y"], N 10; ["x"; "x"], N 20; ["y"], N 30]))

let test_except_3 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound (Emp #< "x" #< "y"))
    (wrap_error @@ fun () -> A.exec (except ["x"; "y"]) Trie.empty)

let test_in_1 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [["x"; "z"], N 10; ["y"], N 20])
    (wrap @@ fun () -> A.exec (in_ ["x"] (renaming [] ["z"])) (of_list [["x"], N 10; ["y"], N 20]))

let test_in_2 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [["x"; "y"], N 10; ["x"; "w"], N 20; ["y"], N 30])
    (wrap @@ fun () -> A.exec (in_ ["x"] (renaming ["x"] ["w"])) (of_list [["x"; "y"], N 10; ["x"; "x"], N 20; ["y"], N 30]))

let test_in_3 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound (Emp #< "x"))
    (wrap_error @@ fun () -> A.exec (in_ ["x"] any) Trie.empty)

let test_in_4 () =
  let t = of_list [["x"; "y"], N 10; ["x"; "x"], N 20; ["y"], N 30] in
  Alcotest.(check @@ trie data) "ok"
    t
    (wrap @@ fun () -> A.exec (in_ ["x"] (renaming ["x"] ["x"])) t)

let test_renaming_1 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [["z"], N 10; ["y"], N 20])
    (wrap @@ fun () -> A.exec (renaming ["x"] ["z"]) (of_list [["x"], N 10; ["y"], N 20]))

let test_renaming_2 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [["z"; "y"], N 10; ["z"; "x"], N 20; ["y"], N 30])
    (wrap @@ fun () -> A.exec (renaming ["x"] ["z"]) (of_list [["x"; "y"], N 10; ["x"; "x"], N 20; ["y"], N 30]))

let test_renaming_3 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound (Emp #< "x"))
    (wrap_error @@ fun () -> A.exec (renaming ["x"] ["z"]) Trie.empty)

let test_renaming_4 () =
  let t = of_list [["x"; "y"], N 10; ["x"; "w"], N 20; ["y"], N 30] in
  Alcotest.(check @@ trie data) "ok"
    t
    (wrap @@ fun () -> A.exec (renaming ["x"] ["x"]) t)

let test_renaming_5 () =
  let t = of_list [["x"; "y"], N 10; ["x"; "w"], N 20; ["y"], N 30] in
  Alcotest.(check @@ trie data) "ok"
    t
    (wrap @@ fun () -> A.exec (renaming ["x"; "y"] ["x"; "y"]) t)

let test_seq_1 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [["w"], N 10; ["y"], N 20])
    (wrap @@ fun () -> A.exec (seq [renaming ["x"] ["z"]; renaming ["z"] ["w"]]) (of_list [["x"], N 10; ["y"], N 20]))

let test_seq_2 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound Emp)
    (wrap_error @@ fun () -> A.exec (seq [none; any]) (of_list [["x"; "y"], N 10; ["x"; "x"], N 20; ["y"], N 30]))

let test_seq_3 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound Emp)
    (wrap_error @@ fun () -> A.exec (seq [none; none]) Trie.empty)

let test_seq_4 () =
  Alcotest.check_raises "error"
    (WrappedBindingNotFound (Emp #< "x"))
    (wrap_error @@ fun () -> A.exec (seq [renaming ["x"] ["z"]; only ["x"]]) (of_list [["x"], N 10; ["y"], N 20]))

let test_seq_5 () =
  let t = of_list [["x"; "y"], N 10; ["x"; "w"], N 20; ["y"], N 30] in
  Alcotest.(check @@ trie data) "ok"
    t
    (wrap @@ fun () -> A.exec (seq [seq []; seq []]) t)

let test_union_1 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [["x"], U ((Emp #< "x"), N 10, N 10); ["y"], N 20])
    (wrap @@ fun () -> A.exec (union [only ["x"]; except ["x"]; only ["x"]]) (of_list [["x"], N 10; ["y"], N 20]))

let test_union_2 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [["x"], U ((Emp #< "x"), N 10, N 10); ["y"], N 20])
    (wrap @@ fun () -> A.exec (in_ ["x"] (union [only []; only []])) (of_list [["x"], N 10; ["y"], N 20]))

let test_union_3 () =
  Alcotest.(check @@ trie data) "ok"
    (of_list [])
    (wrap @@ fun () -> A.exec (union []) (of_list [["x"], N 10; ["y"], N 20]))

let test_union_4 () =
  let t = of_list [["x"; "y"], N 10; ["x"; "w"], N 20; ["y"], N 30] in
  Alcotest.(check @@ trie data) "ok"
    t
    (wrap @@ fun () -> A.exec (union [seq []]) t)
